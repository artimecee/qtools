...
if (iolen == 0) {
  printf("\nNo response to identification command\n");
  return;
}

if (iobuf[0] != 0) {
  printf("\nInvalid response to identification command\n");
  return;
}
//highlight the chipset code
...
if (chip_id == -1) {	
  printf("\nThe chipset is not recognized, the chipset code is %04x\n",chip_code);
  return;
}
//chipset found
set_chipset(chip_id);
...
//Determining the type and parameters of the flash drive
